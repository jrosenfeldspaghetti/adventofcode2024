...
func main() {
	...
	for _, calculation := range strings.Split(inputStr, "\n") {
		...
		if calculateOption(0, goal, numbersToCalc, add) || calculateOption(0, goal, numbersToCalc, multiply) {
			total += goal
		}
...
func calculateOption(current, goal int, vals []int, operator string) bool {
	...
	if operator == multiply {
		...
		if calculateOption(newCurrent, goal, vals[1:], multiply) || calculateOption(newCurrent, goal, vals[1:], add) {
			return true
		}
	}
	if operator == add {
